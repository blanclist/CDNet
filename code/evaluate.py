...
* 注意:
    ...
    由于一些GTs(例如在 "STERE" 测试集中)的原始分辨率很大，以致于无法通过GPU对其进行并行的指标评测
...
"""
evaluate:
    对给定的预测图, 计算多个评测指标(max/mean F-measure, max/mean E-measure, S-measure and MAE).
...
def evaluate(roots, doc_path, num_thread, pin):
    ...
    for dataset in datasets:
        # 对 "dataset" 的预测图进行评测.
        # 注意，需要指定 suffixes 中 gt 和 pred 的后缀名，才能保证正确地读取数据.
        results = evaluate_dataset(roots=roots[dataset], 
                                   dataset=dataset,
                                   batch_size=1, 
                                   num_thread=num_thread, 
                                   demical=True,
                                   suffixes={'gt': '.png', 'pred': '_pred_0.png'},
                                   pin=pin)
